...
class FSMLogger {
	...
	// HOWARD introduces more completed interface
	inline void set_logger(FSMBasicStream& logger) { this->m_logger = &logger; }
	inline void set_logger(FSMBasicStream *logger) { this->set_logger(*logger); }
	...
	inline FSMLogger& set_minlevel(FSMLevel level) {
	...
	inline FSMLogger& set_deflevel(FSMLevel level) {
...
// HOWARD addition
inline FSMLoggerProxy& logger(const std::string& name, FSMLevel level) {
	return get_logger(name).get_proxy(level); }
